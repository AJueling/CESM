...
def FW_region_plot(quant):
    ...
    #
        for s, sim in enumerate(['HIGH', 'LOW']):
            ...
            axl.barh(y=0  +s/10, width=ov, height=.1, align='edge', color='C4', alpha=1-.3*s)
            axl.barh(y=.25+s/10, width=az, height=.1, align='edge', color='C5', alpha=1-.3*s)
            axl.barh(y=.75+s/10, width=to, height=.1, align='edge', color='C6', alpha=1-.3*s)
            if quant=='SALT':  axl.barh(y=.5 +s/10, width=ed, height=.1, align='edge', color='C7', alpha=.9-.3*s)
            ...
            if i==0:  # temporarily store southern boundary values
                ...
                ax_.bar(x=0  +s/10, y=0, height=dd[f'ov_{sim}']-ov.values, width=.1, align='edge', color='C4', alpha=.9-.3*s)
                ax_.bar(x=.25+s/10, y=0, height=dd[f'az_{sim}']-az.values, width=.1, align='edge', color='C5', alpha=.9-.3*s)
                ax_.bar(x=.75+s/10, y=0, height=dd[f'to_{sim}']-to.values, width=.1, align='edge', color='C6', alpha=.9-.3*s)
            ...
            if quant=='SALT':  # SALT_F
                ...
                axt.bar(x=.55+s/10, height=-d[f'Pmi_Sv']-d[f'Emi_Sv'], width=.1, align='edge', color='C1', alpha=1-.3*s)
                axt.bar(x=.3+s/10 , height=-d[f'Rmi_Sv']             , width=.1, align='edge', color='C2', alpha=1-.3*s)
                axt.bar(x=.0+s/10 , height=-d[f'Tmi_Sv']             , width=.1, align='edge', color='C3', alpha=1-.3*s)
                axt.arrow(y=-d[f'Pmi_Sv']-d[f'Emi_Sv'], x=.6+s/10 , dy=-d[f'Pti_Sv']-d[f'Eti_Sv'], dx=0)
                axt.arrow(y=0                         , x=.62+s/10, dy=-d[f'Pti_Sv']             , dx=0)
                axt.arrow(y=0                         , x=.58+s/10, dy=-d[f'Eti_Sv']             , dx=0)
                axt.arrow(y=-d[f'Rmi_Sv']             , x=.35+s/10, dy=-d[f'Rti_Sv']             , dx=0)
                axt.arrow(y=-d[f'Tmi_Sv']             , x=.05+s/10, dy=-d[f'Tti_Sv']             , dx=0)
...
#     plt.savefig(f'{path_results}/Mov/{["FW","SALT"][q]}_region_budget')
#     ax.arrow(x=0,dx=3)
    return